...
export const generateJourneyMessage = async (state: { patientName: string, eventTitle: string, eventDescription: string, daysPostOp: number, tone: Tone }): Promise<string> => {
    return callGeminiWithRetry(async () => {
        const prompt = `Escreva uma mensagem de WhatsApp para o paciente ${state.patientName}.
        Contexto: Pós-operatório de cirurgia ortopédica (${state.daysPostOp} dias após).
        Evento da Jornada: ${state.eventTitle}.
        Detalhes do evento: ${state.eventDescription}.
        Tom: ${state.tone}.
        
        A mensagem deve ser acolhedora, verificar como o paciente está e dar a orientação específica do evento. 
        Se for uma consulta, lembre-o de confirmar. Se for exercício, motive-o.
        Seja curto e direto (estilo WhatsApp). Sem hashtags.`;
...
export const generateMarketingAnalysis = async (metrics: { spend: number, leads: number, patients: number, revenue: number }): Promise<string> => {
    return callGeminiWithRetry(async () => {
        const prompt = `Atue como um estrategista de Marketing Médico.
        Analise os resultados da campanha:
        - Investimento: R$ ${metrics.spend}
        - Leads: ${metrics.leads}
        - Pacientes Agendados: ${metrics.patients}
        - Receita: R$ ${metrics.revenue}
        
        Forneça um diagnóstico curto (max 3 frases) sobre a saúde da campanha e uma recomendação prática de otimização (ex: melhorar criativo, aumentar verba, qualificar lead).`;
